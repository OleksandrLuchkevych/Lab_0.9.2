...
int BinSearch(Student* p, const int N, const string prizv, const Curs curs, const int IT);
...
{ // забезпечення відображення кирилиці:
	...
	do {
		...
		case 4:
			...
			cout << "третя оцінка: "; cin >> IT;
			cout << endl;
			if ((found = BinSearch(p, N, prizv, curs, IT)) != -1)
...
{ // повертає індекс знайденого елемента або -1, якщо шуканий елемент відсутній
	...
	do {
		m = (L + R) / 2;
		if (p[m].prizv == prizv && p[m].curs == curs && p[m].IT == IT)
			return m;
		if ((p[m].curs < curs)
			||
			(p[m].curs == curs &&
